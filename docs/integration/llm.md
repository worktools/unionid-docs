...
参考会明确告诉模型：查询是 PRQL 风格的有序 pipeline；ADT 应使用穷尽 `match`；期望 enum 类型明确时可写 `Pending`，独立或有歧义时写 `State::Pending`；闭包使用 `value -> expression`，不使用 `|value|`；源码不写分号。生成聚合时只使用 `count/count_distinct/avg/sum/min/max`；需要保留行并增加分组排名时使用带显式 sort 的 `window` 和 `row_number/rank/dense_rank`，不要发明 frame、lag、lead 或窗口 aggregate。生成相关存在查询时只使用 `filter exists { from ... }` 或 `filter not exists { from ... }`，以 `outer.path` 引用外层行，并且只能选择 schema 中已有索引支持的目标键；目标路径与外层路径的类型必须兼容。
