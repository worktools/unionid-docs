...
- `{}` 表达 record、字段集合、match branch、aggregate 字段与多键 sort；`[]` 表达 list；`()` 表达 tuple、调用、precedence 与跨行表达式。
- delimiter 内相邻项用逗号分隔，canonical formatter 保留 trailing comma。
...
Record default 必须是可在 schema 阶段完全检查的 typed literal。缺少字段时逐层补默认值；显式错误值不会退回 default。没有 default 的 option 字段仍必须显式写 `None`。
...
- Unit variant 写 `Pending`，record payload 写 `Running {...}`，位置 payload 写 `Pair(1, "x")`。
...
```bash
unionid fmt file.unid
unionid fmt --check file.unid
```
